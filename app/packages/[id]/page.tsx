...
export default function PackageDetailPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-black to-gray-900">
          ...
          <div className="container mx-auto">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    {package_.images.slice(0, 4).map((image, index) => (
                      <button
                        ...
                        className={`relative overflow-hidden rounded-lg ${
                          currentImageIndex === index ? "ring-2 ring-cyan-400" : ""
                        }`}
                      >
                        <img
                          src={image || `/placeholder.svg?height=100&width=100&query=${package_.destination}-${index}`}
              ...
              <div className="space-y-6">
                <div>
                  <h1 className="text-3xl md:text-4xl font-bold text-white mb-4">{package_.title}</h1>
                  <p className="text-white/80 text-lg leading-relaxed">{package_.description}</p>
                </div>
                ...
                <Card className="glass border-white/10">
                  <CardContent className="p-6">
                    <div className="flex items-center justify-between mb-4">
                      <div>
                        <div className="text-3xl font-bold text-cyan-400">KSh {package_.price.toLocaleString()}</div>
          ...
          <div className="container mx-auto">
            <Tabs defaultValue="itinerary" className="w-full">
              ...
              <TabsContent value="itinerary" className="mt-6">
                <Card className="glass border-white/10">
                  <CardHeader>
                    <CardTitle className="text-white">Day by Day Itinerary</CardTitle>
                  </CardHeader>
                        ...
                        <div className="flex-1">
                          ...
                          <div className="flex flex-wrap gap-2">
                            {day.activities.map((activity, actIndex) => (
                              <Badge key={actIndex} variant="outline" className="border-cyan-400/30 text-cyan-400">
                                {activity}
                              </Badge>
                            ))}
                          </div>
      ...
      {/* Booking Modal */}
      {showBookingModal && package_ && <BookingModal package={package_} onClose={() => setShowBookingModal(false)} />}
    </div>
  )
}

...
export async function GET(
  ...
  try {
    ...
    const includeDestination = request.nextUrl.searchParams.get("includeDestination") === "true"
...
export async function PATCH(
  ...
  try {
      ...
      for (const [key, value] of formData.entries()) {
        if (key !== "image") {
          try {
            ...
            updateData[key as keyof UpdatePackageDTO] = JSON.parse(value as string)
          } catch {
    ...
    if (updateData.startDate) updateDoc.startDate = new Date(updateData.startDate)
    if (updateData.endDate) updateDoc.endDate = new Date(updateData.endDate)
    
    // Handle destination change
    if (updateData.destinationId && updateData.destinationId !== existingPackage.destinationId.toString()) {
    ...
    if (updateData.totalSeats && updateData.totalSeats !== existingPackage.totalSeats) {
      const difference = updateData.totalSeats - existingPackage.totalSeats
      updateDoc.availableSeats = Math.max(0, existingPackage.availableSeats + difference)
    }
    
    // Auto-update status based on seat availability
    if (updateDoc.availableSeats === 0) {
      updateDoc.status = "soldout"
    } else if (existingPackage.status === "soldout" && updateDoc.availableSeats > 0) {

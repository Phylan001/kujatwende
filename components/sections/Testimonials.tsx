      ...
      <section className="py-20 px-4">
        <div className="container mx-auto">
          <div className="text-center mb-12">
            ...
            <p className="text-xl text-white/80 max-w-2xl mx-auto">Be the first to share your adventure experience!</p>
          </div>
        </div>
      </section>
    )
        ...
        <div className="relative max-w-4xl mx-auto">
          ...
          <Card className="glass border-white/10 mb-8 animate-fade-scale">
            <CardContent className="p-8">
              <div className="text-center">
                ...
                <h3 className="text-2xl font-bold text-white mb-4">{reviews[currentIndex]?.title}</h3>
                ...
                <div className="flex items-center justify-center gap-3">
                  ...
                  <div>
                    <p className="text-white font-semibold">{reviews[currentIndex]?.user.name}</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-12">
            {reviews.slice(0, 3).map((review, index) => (
              <Card key={review._id} className="glass border-white/10 hover:border-cyan-400/30 transition-colors">
                <CardContent className="p-6">
                  <div className="flex items-center gap-1 mb-3">{renderStars(review.rating)}</div>
                  <h4 className="text-white font-semibold mb-2 line-clamp-1">{review.title}</h4>
                  <p className="text-white/70 text-sm mb-4 line-clamp-3">{review.comment}</p>
                  <div className="flex items-center gap-2">
                    <div className="w-8 h-8 bg-gradient-to-r from-cyan-400 to-purple-600 rounded-full flex items-center justify-center">
                      <User className="w-4 h-4 text-white" />
                    </div>
                    <span className="text-white/80 text-sm">{review.user.name}</span>
                  </div>
                </CardContent>
              </Card>
            ))}
